...
import { fetchImpactsFromSupabase, importSampleImpactsToSupabase, updateImpactInSupabase, uploadPdfRecullToSupabase, importDetectedPdfPagesV3ToSupabase, fetchRecullsFromSupabase, updateRecullAfterProcessing,
  ...
  mergeImpactWithNextPageInSupabase,
} from "@/lib/clippingDb";
import { renderAndUploadPdfPages } from "@/lib/pdfPages";

type Status = "pendent" | "revisat" | "validat" | "arxivat";
const PIECE_TYPES = ["PENDENT", "ONLINE", "PREMSA", "TV", "RADIO", "REVISTA"] as const;
type MediaType = (typeof PIECE_TYPES)[number];
...
export default function Home() {
  ...
  const filtered = useMemo(() => {
    return impacts.filter((impact) => {
      return (
        ...
        (!pendingTypeFilter || (pendingTypeFilter === "pending" ? isPendingPieceType(impact) : !isPendingPieceType(impact))) &&
        (!pendingMediaFilter || (pendingMediaFilter === "pending" ? isPendingMedia(impact) : !isPendingMedia(impact)))
      );
    });
  }, [impacts, typeFilter, statusFilter, campaignFilter, pendingTypeFilter, pendingMediaFilter]);
  ...
  function validateReview() {
    ...
    if (!validateImpactClassification(reviewSelected)) return;
  ...
  function isPendingPieceType(impact: any) {
    return !impact?.mediaType || impact.mediaType === "PENDENT";
  }

  function isPendingMedia(impact: any) {
    const mediaName = String(impact?.mediaName || "").trim().toLowerCase();
  ...
  function canValidateImpact(impact: any) {
    return !isPendingPieceType(impact) && !isPendingMedia(impact);
  }

  function validateImpactClassification(impact: any) {
    if (!impact) return false;

    if (!canValidateImpact(impact)) {
      alert("No es pot validar aquest impacte: falta classificar el tipus de peça o el mitjà.");
      return false;
    }

    updateImpact(impact.id, { status: "validat" });
    return true;
  }

  async function handleMergeWithNextPage() {
    if (!reviewSelected) return;

    const confirmed = window.confirm(
      `Vols unir aquest impacte amb la pàgina següent?\n\nEs conservarà aquest impacte i s'eliminarà l'impacte de la pàgina següent.`,
    );

    if (!confirmed) return;

    try {
      await mergeImpactWithNextPageInSupabase(reviewSelected.id);
      ...
      setUploadMessage("Pàgines unides correctament.");
    } catch (error) {
      console.error(error);
      alert(error instanceof Error ? error.message : "No s'han pogut unir les pàgines.");
    }
  }
                ...
                {!reviewSelected ? (
                  ...
                  <>
                    ...
                    <div className="reviewActionBar">
                      ...
                      <button onClick={skipReview}>Saltar</button>
                      <button onClick={handleMergeWithNextPage}>
                        Unir amb pàgina següent
                      </button>
                  ...
                  <div className="archiveSelected">
                    ...
                    <div className="archiveSelectedInfo">
                      ...
                      <div className="archiveSelectedActions">
                        ...
                        <button onClick={() => validateImpactClassification(archiveSelected)}>
                          Validar
                        </button>
                      </div>
                    </div>
